...
const UnifiedMenuData = () => {
    ...
    if (isUsingDynamicMenu && loading && !FALLBACK_TO_STATIC) {
        return (
            <React.Fragment>
    ...
    // Show dynamic menu error with option to retry or fallback
    if (isUsingDynamicMenu && error && !FALLBACK_TO_STATIC) {
        return (
            <React.Fragment>
    ...
    if (isUsingDynamicMenu && !error && menuItems.length > 0) {
        return (
            <React.Fragment>
                {/* Optional: Show indicator that dynamic menu is active */}
    ...
    // Use dynamic menu while loading if fallback is enabled
    if (isUsingDynamicMenu && loading && FALLBACK_TO_STATIC) {
        return (
            <React.Fragment>
    ...
    if (shouldFallbackToStatic) {
        return (
            <React.Fragment>
                {/* Show indicator that we're using fallback */}
    ...
    // Default to static menu when dynamic is disabled
    return (
        <React.Fragment>
            {/* Optional: Show indicator in development */}
            {process.env.NODE_ENV === 'development' && !USE_DYNAMIC_MENU && (
